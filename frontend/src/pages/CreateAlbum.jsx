...
const CreateAlbum = () => {
  ...
  useEffect(() => {
    ...
    const fetchMusics = async () => {
      try {
        ...
        setMusics(
          res.data.musics.filter(m => m.artist?._id === user.id) || []
        );
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // ✅ FIXED HERE
      await axios.post(`${API}/api/music/album`, {
        title,
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="form-group">
          ...
          <div style={{
            ...
            {musics.map(music => (
              <div 
                ...
                style={{
                  ...
                  display: 'flex',
                  alignItems: 'center',
                  background: selectedMusics.includes(music._id)
                    ? 'var(--bg-hover)'
                    : 'transparent'
                }}
              >
            ...
            {musics.length === 0 && (
              <p style={{ padding: '12px', color: 'var(--text-subdued)' }}>
                No songs found. Upload some music first!
              </p>
            )}
          </div>
        </div>
